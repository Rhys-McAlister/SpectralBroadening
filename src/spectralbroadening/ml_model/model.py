...
def fixed_var_spectrum(frequencies, intensities, xs, fixed_var):
    ...
    for i in range(len(frequencies)):
        y = (
            intensities[i]
            / (2 * np.pi * fixed_var) ** 0.5
            * np.exp(-((xs - frequencies[i]) ** 2) / (2 * fixed_var))
        )
...
def norm_integration(spectra_sum, xs, norm_range_min, norm_range_max):
    norm_factor = sum(
        [
            j
            for i, j in enumerate(spectra_sum)
            if xs[i] >= norm_range_min and xs[i] < norm_range_max
        ]
    )
